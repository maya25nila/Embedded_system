...
void display_9digit(uint32_t count)
{
	uint32_t divisor=100000000;//to extract each digit
	     
	while(divisor > 0)
	{
		lcd_data((count/divisor) % 10 + '0');//extract 9 th digit from left and print
		divisor = divisor /10 ;//to make extraction continue from  left to right
